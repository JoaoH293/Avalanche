...
body {
  ...
  overflow-x: hidden;
  color: var(--ink);
  background: linear-gradient(180deg, #111111 0%, #17130f 46%, #111111 100%);
...
.brand-mark {
  ...
  border-radius: 8px;
  box-shadow: 0 12px 30px rgba(217, 31, 45, 0.34);
  font-size: 1.45rem;
  font-weight: 900;
}
...
.button.primary {
  color: white;
  background: linear-gradient(135deg, var(--red), var(--orange));
  box-shadow: 0 18px 35px rgba(217, 31, 45, 0.22);
}
...
.choice-button:focus {
  ...
  box-shadow: 0 0 0 3px rgba(255, 107, 34, 0.16);
}
...
.send-button {
  ...
  margin-top: 14px;
  color: white;
  background: linear-gradient(135deg, var(--green), #128c3b);
}
...
@media (max-width: 760px) {
  ...
  .has-cart-items .mobile-order-bar {
    display: grid;
  }
...
const buildWhatsAppMessage = () => {
  const lines = [
    "Olá! Quero fazer um pedido na Avalanche Lanches e Salgados.",
    "",
    "Itens:",
  ];
...
const sendOrder = () => {
  ...
  const url = `https://wa.me/${restaurantWhatsApp}?text=${message}`;
...
elements.cartItems.addEventListener("click", (event) => {
  ...
  const amount = control.dataset.cartAction === "increase" ? 1 : -1;
  changeCartQuantity(control.dataset.itemId, amount);
});
...
elements.cashChangeButtons.addEventListener("click", (event) => {
